...
* **Mandatory:** ```true```
* **Default:** ```N/A```
* **Examples:** ```DEST_NAME="JOD My TEST Object"```, ```DEST_NAME="JOD {ProtocolName}"```
...
* **Mandatory:** ```true```
* **Default:** ```N/A```
* **Examples:** ```DEST_ARTIFACT="JOD-MyDist"```
...
* **Mandatory:** ```true```
* **Default:** ```N/A```
* **Examples:** ```JCP_ID="xxxx-xxxx-xxxx"```

JCP Environment Object's credentials id. A string containing the JCP client id for selected JCP Auth (depends on JCP_ENV). It's mandatory, if not set you can't build JOD Distribution.

##### JCP_SECRET

* **Mandatory:** ```true```
* **Default:** ```N/A```
* **Examples:** ```JCP_SECRET="xxxx-xxxx-xxxx"```

JCP Environment Object's credentials secret. A string containing the JCP client secret for selected JCP Auth (depends on JCP_ENV). It's mandatory, if not set you can't build JOD Distribution.

##### JCP_ENV

* **Mandatory:** ```false```
* **Default:** ```stage```
* **Examples:** ```JCP_ENV="prod"```
...
**NB:** This property can take effect only if in ```JOD_CONFIG_TMPL``` file there are the ```%JCP_ENV_XY%``` placeholders.
...
* **Mandatory:** ```false```
* **Default:** ```""```
* **Examples:** ```JOD_NAME="JOD My Object"```
...
**NB:** This property can take effect only if in ```JOD_CONFIG_TMPL``` file there is the ```%JOD_NAME%``` placeholder.

##### JOD_ID

* **Mandatory:** ```false```
* **Default:** ```""```
* **Examples:** ```JOD_ID="XXXXX-XXXXX-XXXXX"```
...
**NB:** This property can take effect only if in ```JOD_CONFIG_TMPL``` file there is the ```%JOD_ID%``` placeholder.

##### JOD_OWNER

* **Mandatory:** ```false```
* **Default:** ```"00000-00000-00000"```
* **Examples:** ```DEST_NAME="XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX"```
...
**NB:** This property can take effect only if in ```JOD_CONFIG_TMPL``` file there is the ```%JOD_OWNER%``` placeholder.

##### JOD_CONFIG_TMPL

* **Mandatory:** ```false```
* **Default:** ```"dists/configs/jod_TMPL.yml"```
* **Examples:** ```JOD_CONFIG_TMPL="dists/configs/jod_TMPL_test.yml"```
...
* **Examples:** ```JOD_CONFIG_LOGS_TMPL="dists/configs/log4j2_TMPL_test.xml"```
...
* **Examples:** ```JOD_STRUCT="dists/configs/struct_test.jod"```
...
* **Mandatory:** ```false```
* **Default:** ```True```
* **Examples:** ```JOD_LOCAL_ENABLED="False"```

Enable/Disable JOD Local Communication, default true.

**NB:** This property can take effect only if in ```JOD_CONFIG_TMPL``` file there is the ```%JOD_LOCAL_ENABLED%``` placeholder.

##### JOD_CLOUD_ENABLED

* **Mandatory:** ```false```
* **Default:** ```True```
* **Examples:** ```JOD_CLOUD_ENABLED="False"```
...
* **Examples:** ```JOD_EXEC_PULLERS="http://com.example.jod.executors.MyPuller"```
...
* **Examples:** ```JOD_EXEC_LISTENERS="file://com.example.jod.executors.MyListener"```
...
* **Examples:** ```JOD_EXEC_EXECUTORS="shell://com.example.jod.executors.MyExecutor"```
