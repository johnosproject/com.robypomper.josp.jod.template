...
(Get-Content "$JOD_DIST_DIR/$JOD_CONFIG_TMPL") | Foreach-Object {
    $_ -replace '%JCP_ENV_API%', "$JCP_ENV_API" `
    -replace '%JCP_ENV_AUTH%', "$JCP_ENV_AUTH" `
    -replace '%JCP_ID%', "$JCP_ID" `
    -replace '%JCP_SECRET%', "$JCP_SECRET" `
    -replace '%JOD_NAME%', "$JOD_NAME" `
    -replace '%JOD_ID%', "$JOD_ID" `
    -replace '%JOD_ID_HW%', "$JOD_ID_HW" `
    -replace '%JOD_EXEC_PULLERS%', "$JOD_EXEC_PULLERS" `
    -replace '%JOD_EXEC_LISTENERS%', "$JOD_EXEC_LISTENERS" `
    -replace '%JOD_EXEC_EXECUTORS%', "$JOD_EXEC_EXECUTORS" `
    -replace '%JOD_OWNER%', "$JOD_OWNER" `
    -replace '%JOD_LOCAL_ENABLED%', "$JOD_LOCAL_ENABLED" `
    -replace '%JOD_CLOUD_ENABLED%', "$JOD_CLOUD_ENABLED" `
    } | Set-Content "$DEST_DIR/configs/jod.yml"

logDeb "Generate JOD logs configs 'log4j2.xml' file"
(Get-Content "$JOD_DIST_DIR/$JOD_CONFIG_LOGS_TMPL") | Foreach-Object {
    $_ -replace '%JOD_VER%', "$JOD_VER" `
