...
#
# $1 JOD configs.sh script's full path
setupJODScriptConfigs() {
  JOD_SCRIPT_CONFIG=$1
  #[ ! -f $JOD_SCRIPT_CONFIG ] && logFat "JOD script config file not found ($JOD_SCRIPT_CONFIG), exit" $ERR_LOAD_SCRIPT_CONFIG
  #execScriptConfigs $JOD_SCRIPT_CONFIG
  [ -f $JOD_SCRIPT_CONFIG ] && execScriptConfigs $JOD_SCRIPT_CONFIG
  ...
  if [ -z "$JOD_NAME" ]; then
    if [ "$OS_TYPE" = "MacOS" ]; then
      HASH=$(echo -n "$JOD_DIR" | md5)
    elif [ "$OS_TYPE" = "Unix" ]; then
      HASH=$(echo -n "$JOD_DIR" | shasum | awk '{print $1}')
    # All others
    else
      HASH=$(echo -n "$JOD_DIR" | shasum | awk '{print $1}')
    fi
    export JOD_NAME="$(basename "$JOD_DIR")-$HASH"
  fi
  export JOD_NAME_DOT=$(echo "$JOD_NAME" | tr '[:upper:]' '[:lower:]' | sed -e 's/\(.*\)/\1/' -e 's| |\.|g')

  # Get JOD_YML
  if [ -z "$JOD_YML" ]; then
    export JOD_YML="$JOD_DIR/configs/jod.yml"
  fi
